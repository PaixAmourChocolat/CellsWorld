...
void StateManager::draw()
{
	...
	for ( ; it != m_states.end() ; ++it)
	{
		m_sharedContext->m_window->getRenderWindow()->setView(it->second->getView());
		it->second->draw();
	}
}
...
void StateManager::switchTo(StateName name)
{
	...
	if (hasState(name))
	{
		...
		temp->activate();
		m_sharedContext->m_window->getRenderWindow()->setView(temp->getView());
...
void StateManager::createState(StateName name)
{
	...
	std::unique_ptr<StateBase> state (found->second());
	state->getView() = m_sharedContext->m_window->getRenderWindow()->getDefaultView();
	
	m_states.emplace_back(name, std::move(state));
